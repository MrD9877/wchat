...
export default function ChatpageInput({ popTost, setChat, room, setKeyBoardHeight, setTextMessage, textMessage }) {
  ...
  const sendMsg = async (e) => {
    ...
    handleIndexDb(textMessage, room, undefined, userId);
    const accessToken = getCookie("accessToken");
    socket.emit("private message", room, { message: textMessage, accessToken });
    ...
    <div className="h-[8%]">
      ...
      <div className="flex justify-between  w-screen px-4 gap-2">
        <div className="flex px-4 py-2 gap-3 bg-white rounded-3xl items-center justify-between w-[80%]">
          ...
          {/* input  */}
          <input onFocus={() => setKeyBoardHeight(0)} ref={textInput} value={textMessage ? textMessage : ""} onChange={(e) => setTextMessage(e.target.value)} className="w-[70%] outline-none px-2 text-lg h-auto overflow-y-scroll" accept="image/*" />
          {/* attachment  */}
          <div>
